...
function getNormalizedColor(color: number | string) {
  let match;
  ...
  if (names.hasOwnProperty(color)) {
    return names[color as keyof typeof names];
  }
...
function toArray(arrayLike: string[]): string[] {
  return Array.prototype.slice.call(arrayLike, 0);
}

function call(...args: string[]) {
  return '\\(\\s*(' + toArray(args).join(')\\s*,\\s*(') + ')\\s*\\)';
}
...
getNormalizedColor.rgba = rgba;

export default getNormalizedColor;
